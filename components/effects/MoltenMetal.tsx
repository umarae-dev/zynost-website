 ...
 * "liquid metal" shader field, unchanged algorithm from the canonical
 * source, typed for this codebase. Themed to Zynost's own violet/sky
 * palette by whoever mounts it, rather than the component's default
 * purple/pink. */
export default function MoltenMetal({
  ...
  useEffect(() => {
    ...
    // WebGL2 context/shader-compile failures (driver quirks, blocked GPU,
    // etc.) previously threw uncaught inside this effect — React treats an
    // effect throw as an error-boundary-triggering failure, which could
    // take out more of the page than just this background. Wrapping setup
    // means a failure here just quietly renders nothing instead.
    try {
